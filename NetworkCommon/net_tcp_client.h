...
namespace network
{
	namespace tcp
	{
		...
		inline std::string Client::PrintSocketData(struct addrinfo* p)
		{
		...
		inline SOCKET Client::CreateSocket()
		{
			...
			for (p = servinfo; p != nullptr; p = p->ai_next)
			{
				...
				if (connect(sockfd, p->ai_addr, p->ai_addrlen) == INVALID_SOCKET)
				{
					continue;
				}
			...
			setsockopt(sockfd, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&enable), sizeof(enable));
